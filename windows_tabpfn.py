...
print(f"Maximum follow-up duration : {df[year_column].max():.2f} years，This will generate {max_year} annual indicator columns.")
...
if not example_check.empty:
    print("Precision test (samples with onset time around 2.1 years):")
    print(example_check)
print(f"\nFinal data frame shape: {df.shape}")
...
primary_cols = [c for c in cohort_data.columns if c not in ['n_eid', 'dementia']]
...
for window_name, target_col in window_config.items():
    print(f"\n{'='*30} window : {window_name} ({target_col}) {'='*30}")

    for model_label, feature_list in models_config.items():
        print(f"\n>>> Executing : {model_label}")
        ...
        print(f"\n[{model_label} | {window_name}] Performance demonstration:")
...
print("\nThe internal and external validation and indicator summary for all windows have been completed.")
